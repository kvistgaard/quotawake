...
if ($Uninstall) {
  ...
  Write-Host "Removed: state files, '$FireTaskName', '$SessionsFireTaskName', '$LogonTaskName'."
  exit 0
}

if ($Install) {
  Register-LogonTask
  Write-Host "Installed logon reconciler task '$LogonTaskName' (runs hidden at logon, exits instantly when nothing is pending)."
  exit 0
}
